...
TEST_F(QueueTest, BasicWriteRead) {
    ...
    // Write
    std::byte* write_ptr = queue.write(msg, msg_len);
...
TEST_F(QueueTest, AutoExpansion) {
    ...
    std::byte* ptr1 = queue.write(msg1, msg1_len);
    ...
    std::byte* ptr2 = queue.write(msg2, msg2_len);
...
TEST_F(QueueTest, MultipleOperations) {
    ...
    for (const auto& msg : messages) {
        std::byte* ptr = queue.write(msg.c_str(), msg.size());
...
TEST_F(QueueTest, EdgeCases) {
    ...
    std::byte* ptr = queue.write("test", 0);
...
TEST_F(QueueTest, LargeData) {
    ...
    std::byte* ptr = queue.write(large_data.data(), large_data.size());
    ASSERT_NE(ptr, nullptr);
...
TEST_F(QueueTest, ChainExpansion) {
    ...
    queue.write(data3.data(), data3.size());
...
TEST_F(QueueTest, ProducerConsumer) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Produce
        queue.write(msg.c_str(), msg.size());
        queue.commit_write(msg.size());
...
TEST_F(QueueTest, Multithreaded) {
    ...
    std::thread producer([&]() {
        for (int i = 0; i < num_messages; ++i) {
            ...
            bool written = false;
            while (!written) {
                std::byte* ptr = queue.write(msg, msg_len);
                if (ptr != nullptr) {
                    queue.commit_write(msg_len);
                    written = true;
                } else {
...
TEST_F(RingBytesTest, BasicOperations) {
    ...
    // Write
    std::byte* write_ptr = ring.write(msg, msg_len);
    ASSERT_NE(write_ptr, nullptr);
...
TEST_F(RingBytesTest, CapacityLimits) {
    ...
    std::byte* ptr = ring.write(data.data(), 50);
    ASSERT_NE(ptr, nullptr);
...
TEST_F(RingBytesTest, WrapAround) {
    ...
    std::byte* ptr = ring.write(data2.data(), 25);
    ASSERT_NE(ptr, nullptr);
